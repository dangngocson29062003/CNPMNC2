...
const LocationScreen = ({ navigation, route }) => {
  ...
  const movie = moviesData.find((meal) => meal.id === id);
  ...
  const payButtonHandler = () => {
    ...
    selectedSeats.forEach((seat) => {
      ...
      const seatIndex = updateRows[rowIndex].seats.findIndex(
        (s) => s.seat == seat.seat
      ); //s.seat laf seat trong mang đầu : 1,2,3,4,..
    ...
    <>
      <SafeAreaView style={styles.rootContainer}>
        ...
        <View style={styles.bottomContainer}>
          <View style={styles.separateContainer}>
            ...
            <View>
              <Text style={[styles.textLight, { marginTop: 5 }]}>
                {movie.language}
              </Text>
            </View>
            <Text style={[styles.textLight, { marginTop: 5 }]}>
      ...
      {selectedSeats.length > 0 ? (
        ...
        >
          ...
          >
            {selectedSeats.length} ghế đã chọn {seatNumbers}
          </Text>
        </Pressable>
      ) : (
